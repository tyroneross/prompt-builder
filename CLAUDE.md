# CLAUDE.md — prompt-builder plugin notes
...
```
prompt-builder/
├── .claude-plugin/plugin.json          # Plugin manifest (name: prompt-builder)
├── skills/prompt-builder/            # The single skill. Do not rename — callers depend on this name.
...
├── commands/                            # Thin slash-command wrappers (/prompt-builder:*)
├── evals/
...
1. **The skill name is `prompt-builder`**. The plugin name is `prompt-builder`. Callers (agents, tools) invoke `Skill: prompt-builder`. Slash commands use the plugin prefix `/prompt-builder:<cmd>`.
...
- **build-loop**: Can invoke `prompt-builder` inside its Phase 4 (code-gen) to optimize sub-prompts used for specialist agents. Memory note flagged: "Do not wire in yet" until user confirms.
- **bookmark**: No integration needed. `.prompt-builder/` library is independent of `.bookmark/`.
- **ibr**: No integration. IBR is for UI validation; prompt-builder is for prompt authoring.
...
- The library (`.prompt-builder/`) has no built-in garbage collection. Old versions accumulate — prune manually.

## Future work (deferred)

- `/prompt-builder:diff <id> <v1> <v2>` — textual diff of saved versions
