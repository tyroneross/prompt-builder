...
List saved prompts in `.pretty-prompts/prompts/`.

**User arguments**: {{ARGUMENTS}} (optional filter; matches against prompt_id substring)

**Behavior**:

1. Check if `.pretty-prompts/prompts/` exists in the current working directory. If not, report: `No prompt library in this project. Run /pretty-prompts:save <id> to start one.`

2. For each subdirectory under `.pretty-prompts/prompts/`:
...
6. Offer next actions:
   - `/pretty-prompts:compare <id-a> <id-b>` — compare any two
   - Read a specific saved prompt: `cat .pretty-prompts/prompts/<id>/v{n}.md`
