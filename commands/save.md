...
Save the last-optimized prompt (or one the user provides) to `.pretty-prompts/prompts/<id>/`.
...
2. Determine what to save:
   - If the user just ran `/pretty-prompts:optimize` or `/pretty-prompts:compare` in this conversation, use that output.
   - Otherwise, ask the user to paste the optimized prompt + its CONFIG line.

3. Read any existing `.pretty-prompts/prompts/<id>/` to find the highest version number. The new version is `v{n+1}`. If the directory doesn't exist, create it and use `v1`.

4. Write two files:
   - `.pretty-prompts/prompts/<id>/v{n}.md` — the OPTIMIZED_PROMPT block (just the prompt content, copy-ready)
   - `.pretty-prompts/prompts/<id>/v{n}.meta.json` — structured metadata:
     ```json
     {
       "prompt_id": "<id>",
       "version": "v{n}",
...
6. Report the saved path and confirm with a short summary:
   - `Saved: .pretty-prompts/prompts/<id>/v{n}.md (score 21/25)`
