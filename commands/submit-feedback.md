...
description: Report a bug or request a feature for the prompt-builder plugin — drafts a GitHub issue and files it only after you approve
argument-hint: "[what broke, or what you wish it did]"
---

# Submit prompt-builder feedback

File the user's report as a GitHub issue on `tyroneross/prompt-builder`. Issues are this
...
3. **Draft the issue.** Use the template for the classified kind:
   ...
   **Bug**
   ...
   ## Steps to reproduce
   ## Environment
   plugin: prompt-builder @ <sha> · claude: <version> · os: <uname -sm>
   ```
...
5. **File it**, only after approval:
   ...
   ```bash
   gh issue create --repo tyroneross/prompt-builder \
...
6. **If `gh` is missing or unauthenticated**, do not fail — print the URL so the user
   ...
   https://github.com/tyroneross/prompt-builder/issues/new
